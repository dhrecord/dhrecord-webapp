...
if(isset($_GET['vkey'])){
	...
	if($resultSet->num_rows == 1)
	{
		 ...
		 if ($update)
		 {
			...
			$stmt = mysqli_prepare($conn, "insert into registeredPatient(fullName, nricNumber, contactNumber, email, address, medConditions, drugAllergies, users_ID, attendingDoctor, familyTag) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			mysqli_stmt_bind_param($stmt, "sssssssssi", $row['fullName'], $row['nricNumber'], $row['contactNumber'], $row['email'], $row['address'], $row['medConditions'], $row['drugAllergies'], $row1['ID'], $row['attendingDoctor'], $familyTagZero);
			mysqli_stmt_execute($stmt);
